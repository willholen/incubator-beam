...
def write_pattern(lines_per_file, no_data=False, return_filenames=False):
  ...
  for i in range(len(lines_per_file)):
    file_name, data = write_data(lines_per_file[i],
                                 no_data=no_data,
                                 directory=temp_dir,
                                 prefix='mytemp')
    if return_filenames:
...
class TextSourceTest(unittest.TestCase):
  ...
  def _run_read_test(self,
    ...
    source = TextSource(file_or_pattern, 0, compression, True,
                        coders.StrUtf8Coder(), buffer_size)
  ...
  def test_read_single_file_single_line_no_eol(self):
    file_name, expected_data = write_data(1,
                                          eol=EOL.LF_WITH_NOTHING_AT_LAST_LINE)
  ...
  def test_read_empty_single_file(self):
    file_name, written_data = write_data(1,
                                         no_data=True,
                                         eol=EOL.LF_WITH_NOTHING_AT_LAST_LINE)
  ...
  def test_read_single_file_last_line_no_eol_gzip(self):
    ...
    assert len(expected_data) == TextSourceTest.DEFAULT_NUM_RECORDS
    self._run_read_test(gzip_file_name,
                        expected_data,
  ...
  def test_read_all_single_file(self):
    ...
    assert len(expected_data) == 5
    with TestPipeline() as pipeline:
      pcoll = pipeline | 'Create' >> Create([file_name
  ...
  def test_read_from_text_with_file_name_file_pattern(self):
    pattern, expected_data = write_pattern(lines_per_file=[5, 5],
  ...
  def test_read_all_file_pattern(self):
    ...
    assert len(expected_data) == 40
    with TestPipeline() as pipeline:
      pcoll = (pipeline | 'Create' >> Create([pattern]) |
               'ReadAll' >> ReadAllFromText())
      assert_that(pcoll, equal_to(expected_data))
  ...
  def test_read_all_many_file_patterns(self):
    ...
    expected_data.extend(expected_data3)
    with TestPipeline() as pipeline:
      pcoll = pipeline | 'Create' >> Create([pattern1, pattern2, pattern3
                                            ]) | 'ReadAll' >> ReadAllFromText()
      assert_that(pcoll, equal_to(expected_data))
  ...
  def test_read_bzip2_concat(self):
    with TempDir() as tempdir:
      ...
      with open(bzip2_file_name1, 'rb') as src, open(final_bzip2_file,
                                                     'wb') as dst:
        dst.writelines(src.readlines())

      with open(bzip2_file_name2, 'rb') as src, open(final_bzip2_file,
                                                     'ab') as dst:
        dst.writelines(src.readlines())

      with open(bzip2_file_name3, 'rb') as src, open(final_bzip2_file,
                                                     'ab') as dst:
        dst.writelines(src.readlines())
  ...
  def test_read_all_gzip(self):
    ...
    with TempDir() as tempdir:
      ...
      with gzip.GzipFile(file_name, 'wb') as f:
        f.write('\n'.join(lines).encode('utf-8'))
      with TestPipeline() as pipeline:
        pcoll = (pipeline | Create([file_name]) | 'ReadAll' >>
