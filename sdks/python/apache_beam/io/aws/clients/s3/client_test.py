...
class ClientErrorTest(unittest.TestCase):
  ...
  def test_get_object_metadata(self):
    ...
    self.assertRaises(messages.S3ClientError, self.client.get_object_metadata,
                      request)
  ...
  def test_get_range_nonexistent(self):
    ...
    object = self.test_path + 'nonexistent_file_doesnt_exist'
    request = messages.GetRequest(self.test_bucket, object)
    self.assertRaises(messages.S3ClientError, self.client.get_range, request, 0,
                      10)
  ...
  def test_get_range_bad_start_end(self):
    ...
    response = self.client.get_range(messages.GetRequest(bucket, object), -10,
                                     20)
    self.assertEqual(response, contents)

    response = self.client.get_range(messages.GetRequest(bucket, object), 20,
                                     10)
    self.assertEqual(response, contents)
  ...
  def test_upload_part_nonexistent_upload_id(self):
    ...
    request = messages.UploadPartRequest(self.test_bucket, object, upload_id,
  ...
  def test_copy_nonexistent(self):
    ...
    request = messages.CopyRequest(self.test_bucket, src_key, self.test_bucket,
                                   dest_key)
  ...
  def test_complete_multipart_upload_too_small(self):
    ...
    request_1 = messages.UploadPartRequest(self.test_bucket, object, upload_id,
                                           part_number, contents_1)
    response_1 = self.client.upload_part(request_1)

    part_number = 2
    contents_2 = os.urandom(1024)
    request_2 = messages.UploadPartRequest(self.test_bucket, object, upload_id,
                                           part_number, contents_2)
