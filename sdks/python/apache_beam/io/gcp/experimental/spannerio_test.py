...
class SpannerReadTest(unittest.TestCase):
  ...
  def test_read_with_query_batch(self, mock_batch_snapshot_class,
    ...
    read = (pipeline | 'read' >> ReadFromSpanner(TEST_PROJECT_ID,
                                                 TEST_INSTANCE_ID,
    ...
    readpipeline = (
        pipeline | 'create reads' >> beam.Create(ro) |
  ...
  def test_read_with_transaction(self, mock_batch_snapshot_class,
    ...
    read_query = (
        p |
        'with query' >> ReadFromSpanner(project_id=TEST_PROJECT_ID,
                                        instance_id=TEST_INSTANCE_ID,
                                        database_id=_generate_database_name(),
    ...
    read_table = (
        p |
        'with table' >> ReadFromSpanner(project_id=TEST_PROJECT_ID,
                                        instance_id=TEST_INSTANCE_ID,
                                        database_id=_generate_database_name(),
                                        transaction=transaction,
                                        table="users",
    ...
    read_indexed_table = (
        p |
        'with index' >> ReadFromSpanner(project_id=TEST_PROJECT_ID,
                                        instance_id=TEST_INSTANCE_ID,
                                        database_id=_generate_database_name(),
                                        transaction=transaction,
                                        table="users",
    ...
    read_pipeline = (p | 'create read operations' >> beam.Create(ro) |
    ...
    with self.assertRaises(ValueError):
      # Test the exception raised when user passes the read operations in the
      # constructor and also in the pipeline.
      _ = (p | 'create read operations2' >> beam.Create(ro) |
