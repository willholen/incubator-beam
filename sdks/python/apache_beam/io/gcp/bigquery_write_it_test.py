...
class BigQueryWriteIntegrationTests(unittest.TestCase):
  ...
  def create_table(self, table_name):
    ...
    table = bigquery.Table(tableReference=bigquery.TableReference(
        projectId=self.project, datasetId=self.dataset_id, tableId=table_name),
                           schema=table_schema)
    request = bigquery.BigqueryTablesInsertRequest(projectId=self.project,
                                                   datasetId=self.dataset_id,
                                                   table=table)
  ...
  def test_big_query_write_without_schema(self):
    ...
    pipeline_verifiers = [
        BigqueryFullResultMatcher(project=self.project,
                                  query="SELECT bytes, date, time FROM %s" %
                                  table_id,
                                  data=[(
                                      b'xyw',
    ...
    with beam.Pipeline(argv=args) as p:
      ...
      (p | 'create' >> beam.Create(input_data) |
       'write' >> beam.io.WriteToBigQuery(
           table_id, write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND)
      )
