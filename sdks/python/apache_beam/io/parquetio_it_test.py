...
class TestParquetIT(unittest.TestCase):
  ...
  def _sum_verifier(init_size, data_size, x):
    ...
    if x != expected:
      raise BeamAssertException("incorrect sum: expected(%d) actual(%d)" %
                                (expected, x))
    return []
  ...
  def _count_verifier(init_size, data_size, x):
    ...
    if count != expected_count:
      raise BeamAssertException("incorrect count(%s): expected(%d) actual(%d)" %
  ...
  def _verify_data(self, pcol, init_size, data_size):
    ...
    v1 = (
        read | 'get_number' >> Map(lambda x: x['number']) |
        'sum_globally' >> CombineGlobally(sum) | 'validate_number' >>
        FlatMap(lambda x: TestParquetIT._sum_verifier(init_size, data_size, x)))
    v2 = (read | 'make_pair' >> Map(lambda x: (x['name'], x['number'])) |
          'count_per_key' >> Count.PerKey() | 'validate_name' >> FlatMap(
              lambda x: TestParquetIT._count_verifier(init_size, data_size, x)))
    _ = ((v1, v2, pcol) | 'flatten' >> Flatten() | 'reshuffle' >> Reshuffle() |
  ...
  def _generate_data(self, p, output_prefix, init_size, data_size):
    ...
    lines = (p | 'create' >> Create(init_data) |
