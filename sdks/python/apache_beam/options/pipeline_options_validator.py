...
class PipelineOptionsValidator(object):
  ...
  OPTIONS = [
      DebugOptions, GoogleCloudOptions, SetupOptions, StandardOptions,
  ...
  def is_service_runner(self):
    ...
    is_service_runner = (self.runner is not None and type(
        self.runner).__name__ in ['DataflowRunner', 'TestDataflowRunner'])

    dataflow_endpoint = (
        self.options.view_as(GoogleCloudOptions).dataflow_endpoint)
    is_service_endpoint = (dataflow_endpoint is not None and
                           self.is_full_string_match(self.ENDPOINT_PATTERN,
                                                     dataflow_endpoint))
  ...
  def validate_cloud_options(self, view):
    ...
    if project is None:
      ...
      if self.is_full_string_match(self.PROJECT_NUMBER_PATTERN, project):
        ...
        errors.extend(self._validate_error(self.ERR_INVALID_PROJECT_ID,
                                           project))
    if view.update:
    ...
    if view.transform_name_mapping:
      ...
      for _, (key, value) in enumerate(view.transform_name_mapping.items()):
        if not isinstance(key, (str, unicode)) \
          ...
          errors.extend(
              self._validate_error(self.ERR_INVALID_TRANSFORM_NAME_MAPPING, key,
                                   value))
          break
    return errors
  ...
  def validate_test_matcher(self, view, arg_name):
    ...
    try:
      ...
      if not isinstance(matcher, BaseMatcher):
        errors.extend(
            self._validate_error(self.ERR_INVALID_TEST_MATCHER_TYPE, matcher,
                                 arg_name))
