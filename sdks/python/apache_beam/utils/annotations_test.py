...
class AnnotationTests(unittest.TestCase):
  ...
  def test_deprecated_custom_no_replacements(self):
    ...
    with warnings.catch_warnings(record=True) as w:
      ...
      self.check_custom_annotation(warning=w,
                                   ...
                                   replacements',
                                   annotation_type='experimental',
                                   intended_message=strCustom.replace(
                                       '%since%', strSince).replace(
                                           '%current%',
                                           '').replace('%extra%', ''))
  ...
  def test_experimental_with_current_message_custom(self):
    with warnings.catch_warnings(record=True) as w:
      ...
      @experimental(current=strCurrent,
                    extra_message=strExtra,
                    custom_message=strCustom)
  ...
  # helper function
  def check_annotation(self, warning, warning_type, obj_name, annotation_type,
                       label_check_list):
