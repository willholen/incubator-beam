...
class ConsumerSet(Receiver):
  ...
  def create(
    ...
    # type: (...) -> ConsumerSet
    if len(consumers) == 1:
      return SingletonConsumerSet(counter_factory, step_name, output_index,
                                  consumers, coder)
    else:
      return ConsumerSet(counter_factory, step_name, output_index, consumers,
                         coder)
  ...
  def __init__(
    ...
    self.opcounter = opcounters.OperationCounters(counter_factory, step_name,
                                                  coder, output_index)
    # Used in repr.
...
class SingletonConsumerSet(ConsumerSet):
  ...
  def __init__(self, counter_factory, step_name, output_index, consumers,
    ...
    super(SingletonConsumerSet, self).__init__(counter_factory, step_name,
                                               output_index, consumers, coder)
...
class Operation(object):
  ...
  def progress_metrics(self):
    ...
    return beam_fn_api_pb2.Metrics.PTransform(
        ...
        ProcessedElements(measured=beam_fn_api_pb2.Metrics.PTransform.Measured(
            total_time_spent=(self.scoped_start_state.sampled_seconds() +
  ...
  def execution_time_monitoring_infos(self, transform_id):
    # type: (str) -> Dict[FrozenSet, metrics_pb2.MonitoringInfo]
    total_time_spent_msecs = (self.scoped_start_state.sampled_msecs_int() +
...
class ImpulseReadOperation(Operation):
  ...
  def __init__(self, name_context, counter_factory, state_sampler, consumers,
               source, output_coder):
    super(ImpulseReadOperation, self).__init__(name_context, None,
                                               counter_factory, state_sampler)
    self.source = source
    self.receivers = [
        ConsumerSet.create(self.counter_factory, self.name_context.step_name, 0,
...
class _TaggedReceivers(dict):
  ...
  def __missing__(self, tag):
    self[tag] = receiver = ConsumerSet(self._counter_factory, self._step_name,
                                       tag, [], None)
    return receiver
...
class DoOperation(Operation):
  ...
  def setup(self):
    ...
    with self.scoped_start_state:
      ...
      self.dofn_receiver = (self.dofn_runner if isinstance(
          self.dofn_runner, Receiver) else DoFnRunnerReceiver(self.dofn_runner))
  ...
  def process_timer(self, tag, windowed_timer):
    ...
    self.dofn_receiver.process_user_timer(timer_spec, key,
                                          windowed_timer.windows[0],
                                          timer_data['timestamp'])
    ...
    with self.lock:
      if self.element_start_output_bytes is not None:
        ...
        if progress is not None:
          return progress.with_completed(self._total_output_bytes() -
...
class CombineOperation(Operation):
  ...
  def __init__(self, name_context, spec, counter_factory, state_sampler):
    super(CombineOperation, self).__init__(name_context, spec, counter_factory,
                                           state_sampler)
    ...
    self.phased_combine_fn = (PhasedCombineFnExecutor(self.spec.phase, fn, args,
                                                      kwargs))
...
class PGBKOperation(Operation):
  ...
  def __init__(self, name_context, spec, counter_factory, state_sampler):
    super(PGBKOperation, self).__init__(name_context, spec, counter_factory,
                                        state_sampler)
  ...
  def flush(self, target):
    ...
    for ix, (kw, vs) in enumerate(list(self.table.items())):
      ...
      windowed_value = WindowedValue((key, output_value), vs[0].timestamp,
                                     windows)
      self.output(windowed_value)
...
class PGBKCVOperation(Operation):
  ...
  def __init__(self,
               ...
               state_sampler,
               windowing=None):
    super(PGBKCVOperation, self).__init__(name_context, spec, counter_factory,
                                          state_sampler)
...
def create_operation(name_context,
  ...
  if isinstance(spec, operation_specs.WorkerRead):
    ...
    if is_streaming:
      ...
      op = BatchGroupAlsoByWindowsOperation(name_context, spec, counter_factory,
                                            state_sampler)
    ...
    op = ReifyTimestampAndWindowsOperation(name_context, spec, counter_factory,
                                           state_sampler)
  else:
...
class SimpleMapTaskExecutor(object):
  ...
  def execute(self):
    ...
    for name_context, spec in zip(self._map_task.name_contexts,
      ...
      op = create_operation(name_context,
                            spec,
