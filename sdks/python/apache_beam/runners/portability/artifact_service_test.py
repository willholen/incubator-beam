...
class BeamFilesystemArtifactServiceTest(unittest.TestCase):
  ...
  def test_basic(self):
    self.run_staging_test(self._service, self._service)
  ...
  def run_staging_test(self, staging_service, retrieval_service):
  ...
  def test_concurrent_requests(self):
    ...
    def delayed_data(data, index, max_msecs=1):
      ...
      return ('%s%d' % (data, index)).encode('ascii')

    def session(index):
      return 'session%d' % (index % 7)
