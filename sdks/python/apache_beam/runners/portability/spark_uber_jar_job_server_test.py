...
class SparkUberJarJobServerTest(unittest.TestCase):
  ...
  def test_get_server_spark_version(self, http_mock):
    http_mock.get(
        ...
        json={
            ...
            "message":
                "Missing protocol version. Please submit requests through "
  ...
  def test_end_to_end(self, http_mock):
    ...
    with temp_name(suffix='fake.jar') as fake_jar:
      ...
      http_mock.get('http://host:6066/v1/submissions/status/submission-id', [
      ...
      self.assertEqual([s.state for s in state_stream], [
          beam_job_api_pb2.JobState.STOPPED, beam_job_api_pb2.JobState.RUNNING,
          beam_job_api_pb2.JobState.RUNNING, beam_job_api_pb2.JobState.FAILED
      ])
      ...
      self.assertEqual([get_item(m) for m in message_stream], [
          ...
          beam_job_api_pb2.JobMessage(message_id='message0',
                                      time='0',
