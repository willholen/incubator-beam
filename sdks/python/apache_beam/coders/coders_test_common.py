...
class CodersTest(unittest.TestCase):
  ...
  def tearDownClass(cls):
    standard = set(c for c in coders.__dict__.values()
                   if isinstance(c, type) and issubclass(c, coders.Coder) and
                   'Base' not in c.__name__)
    standard -= set([
        coders.Coder, coders.AvroGenericCoder, coders.DeterministicProtoCoder,
        coders.FastCoder, coders.ProtoCoder, coders.RunnerAPICoderHolder,
  ...
  def test_timer_coder(self):
    ...
    self.check_coder(
        coders.TupleCoder((coders._TimerCoder(coders.VarIntCoder()),)), ({
            'timestamp': timestamp.Timestamp.of(37000),
            'payload': 389
        },))
  ...
  def test_tuple_sequence_coder(self):
    ...
    self.check_coder(coders.TupleCoder((coders.VarIntCoder(), int_tuple_coder)),
                     (1, (1, 2, 3)))
  ...
  def test_windowedvalue_coder_paneinfo(self):
    ...
    test_paneinfo_values = [
        ...
        windowed_value.PaneInfo(True, True, windowed_value.PaneInfoTiming.EARLY,
                                0, -1),
        windowed_value.PaneInfo(True, False,
                                windowed_value.PaneInfoTiming.ON_TIME, 0, 0),
        windowed_value.PaneInfo(True, False,
                                windowed_value.PaneInfoTiming.ON_TIME, 10, 0),
        windowed_value.PaneInfo(False, True,
                                windowed_value.PaneInfoTiming.ON_TIME, 0, 23),
        windowed_value.PaneInfo(False, True,
                                windowed_value.PaneInfoTiming.ON_TIME, 12, 23),
        windowed_value.PaneInfo(False, False,
                                windowed_value.PaneInfoTiming.LATE, 0, 123),
    ]
  ...
  def test_windowed_value_coder(self):
    ...
    # Test nested
    self.check_coder(
        coders.TupleCoder((coders.WindowedValueCoder(coders.FloatCoder()),
                           coders.WindowedValueCoder(coders.StrUtf8Coder()))),
        (windowed_value.WindowedValue(
            1.5, 0, ()), windowed_value.WindowedValue("abc", 10, ('window',))))
  ...
  def test_param_windowed_value_coder(self):
    ...
    self.check_coder(
        coders.ParamWindowedValueCoder(
            payload, [coders.VarIntCoder(),
                      coders.IntervalWindowCoder()]),
        windowed_value.WindowedValue(3, 1, (window.IntervalWindow(11, 21),),
                                     PaneInfo(True, False, 1, 2, 3)),
        windowed_value.WindowedValue(1, 1, (window.IntervalWindow(11, 21),),
                                     PaneInfo(True, False, 1, 2, 3)))
    ...
    self.check_coder(
        coders.TupleCoder(
            (coders.ParamWindowedValueCoder(
                payload, [coders.FloatCoder(),
                          coders.IntervalWindowCoder()]),
             coders.ParamWindowedValueCoder(
                 payload, [coders.StrUtf8Coder(),
                           coders.IntervalWindowCoder()]))),
        (windowed_value.WindowedValue(1.5, 1, (window.IntervalWindow(11, 21),),
                                      PaneInfo(True, False, 1, 2, 3)),
         windowed_value.WindowedValue("abc", 1,
  ...
  def test_length_prefix_coder(self):
    ...
    # Test nested
    self.check_coder(coders.TupleCoder((coder, coder)), (b'', b'a'),
                     (b'bc', b'def'))
  ...
  def test_state_backed_iterable_coder(self):
    ...
    coder = coders.StateBackedIterableCoder(coders.VarIntCoder(),
