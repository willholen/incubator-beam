...
class SideInputsTest(unittest.TestCase):
  ...
  def run_windowed_side_inputs(self,
                               elements,
                               main_window_fn,
                               side_window_fn=None,
    ...
    with self.create_pipeline() as p:
      ...
      side = pcoll | 'WindowSide' >> beam.WindowInto(side_window_fn or
                                                     main_window_fn)
      kw = {}
  ...
  def test_empty_singleton_side_input(self):
    ...
    result = pcol | 'compute' >> beam.FlatMap(my_fn,
                                              beam.pvalue.AsSingleton(side))
  ...
  def test_default_value_singleton_side_input(self):
    ...
    result = pcol | beam.FlatMap(lambda x, s: [x * s],
                                 beam.pvalue.AsSingleton(side, 10))
  ...
  def test_iterable_side_input(self):
    ...
    pcol = pipeline | 'start' >> beam.Create([1, 2])
    side = pipeline | 'side' >> beam.Create([3, 4])  # 2 values in side input.
    result = pcol | 'compute' >> beam.FlatMap(lambda x, s: [x * y for y in s],
  ...
  def test_as_singleton_without_unique_labels(self):
    ...
    main_input = pipeline | 'main input' >> beam.Create([1])
    side_list = pipeline | 'side list' >> beam.Create(a_list)
    results = main_input | beam.Map(lambda x, s1, s2: [x, s1, s2],
  ...
  def test_as_list_twice(self):
    ...
    main_input = pipeline | 'main input' >> beam.Create([1])
    side_list = pipeline | 'side list' >> beam.Create(a_list)
    results = main_input | beam.Map(lambda x, ls1, ls2: [x, ls1, ls2],
  ...
  def test_as_dict_twice(self):
    ...
    results = main_input | beam.Map(lambda x, dct1, dct2: [x, dct1, dct2],
  ...
  def test_flattened_side_input(self):
    ...
    side_input = (pipeline | 'side1' >> beam.Create(['a']),
  ...
  def test_multi_triggered_gbk_side_input(self):
    ...
    test_stream = (p | 'Mixed TestStream' >> TestStream().advance_watermark_to(
        3, tag='main').add_elements(['a1'], tag='main').advance_watermark_to(
    ...
    main_data = (test_stream['main'] | 'Main windowInto' >> beam.WindowInto(
        window.FixedWindows(5),
        accumulation_mode=trigger.AccumulationMode.DISCARDING))

    side_data = (test_stream['side'] | 'Side windowInto' >> beam.WindowInto(
        window.FixedWindows(5),
    ...
    expected_window_to_elements = {
        window.IntervalWindow(0, 5): [('a1', Timestamp(3), [100, 0]),],
