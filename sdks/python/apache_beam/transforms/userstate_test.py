...
class InterfaceTest(unittest.TestCase):
  ...
  def test_good_signatures(self):
    ...
    signature = self._validate_dofn(stateful_dofn)
    expected_specs = (set(
        [TestStatefulDoFn.BUFFER_STATE_1, TestStatefulDoFn.BUFFER_STATE_2]),
                      set([
...
class StatefulDoFnOnDirectRunnerTest(unittest.TestCase):
  ...
  def test_simple_stateful_dofn(self):
    ...
    with TestPipeline() as p:
      test_stream = (TestStream().advance_watermark_to(10).add_elements(
          [1, 2]).add_elements([3]).advance_watermark_to(25).add_elements([4]))
      (p | test_stream | beam.Map(lambda x: ('mykey', x)) |
       beam.ParDo(SimpleTestStatefulDoFn()) | beam.ParDo(self.record_dofn()))
  ...
  def test_clearing_bag_state(self):
    ...
    with TestPipeline() as p:
      ...
      _ = (p | test_stream | beam.ParDo(BagStateClearingStatefulDoFn()) |
  ...
  def test_simple_set_stateful_dofn(self):
    ...
    with TestPipeline() as p:
      ...
      (p | test_stream | beam.Map(lambda x: ('mykey', x)) |
       beam.ParDo(SimpleTestSetStatefulDoFn()) | beam.ParDo(self.record_dofn()))
  ...
  def test_clearing_set_state(self):
    ...
    with TestPipeline() as p:
      ...
      _ = (p | test_stream | beam.ParDo(SetStateClearingStatefulDoFn()) |
           beam.ParDo(self.record_dofn()))
  ...
  def test_simple_stateful_dofn_combining(self):
    ...
    class SimpleTestStatefulDoFn(DoFn):
      BUFFER_STATE = CombiningValueStateSpec('buffer',
                                             IterableCoder(VarIntCoder()),
                                             ToListCombineFn())
      EXPIRY_TIMER = TimerSpec('expiry1', TimeDomain.WATERMARK)
  ...
  def test_timer_output_timestamp(self):
    ...
    with TestPipeline() as p:
      ...
      (p | test_stream | beam.Map(lambda x: ('mykey', x)) |
       ...
       beam.ParDo(TimestampReifyingDoFn()) | beam.ParDo(self.record_dofn()))
  ...
  def test_timer_output_timestamp_and_window(self):
    ...
    class TimerEmittingStatefulDoFn(DoFn):
      ...
      def emit_callback_1(self,
                          ...
                          key=DoFn.KeyParam):
        yield ('timer1-{key}'.format(key=key), int(ts), int(window.start),
               int(window.end))

    pipeline_options = PipelineOptions()
    with TestPipeline(options=pipeline_options) as p:
      test_stream = (TestStream().advance_watermark_to(10).add_elements([1]))
      (p | test_stream | beam.Map(lambda x: ('mykey', x)) | "window_into" >>
       beam.WindowInto(window.FixedWindows(5),
                       accumulation_mode=trigger.AccumulationMode.DISCARDING) |
       beam.ParDo(TimerEmittingStatefulDoFn()) | beam.ParDo(self.record_dofn()))
  ...
  def test_hash_join(self):
    ...
    with TestPipeline() as p:
      ...
      (p | test_stream | beam.ParDo(HashJoinStatefulDoFn()) |
       beam.ParDo(self.record_dofn()))
