...
class BatchElementsTest(unittest.TestCase):
  ...
  def test_grows_to_max_batch(self):
    # Assumes a single bundle...
    with TestPipeline() as p:
      res = (p | beam.Create(range(164)) | util.BatchElements(
  ...
  def test_windowed_batches(self):
    ...
    with TestPipeline() as p:
      res = (p | beam.Create(range(47), reshuffle=False) |
             beam.Map(lambda t: window.TimestampedValue(t, t)) |
             beam.WindowInto(window.FixedWindows(30)) | util.BatchElements(
  ...
  def test_target_duration(self):
    clock = FakeClock()
    batch_estimator = util._BatchSizeEstimator(target_batch_overhead=None,
  ...
  def test_target_overhead(self):
    clock = FakeClock()
    batch_estimator = util._BatchSizeEstimator(target_batch_overhead=.05,
  ...
  def test_variance(self):
    ...
    self.assertGreater(min(stable_set),
                       expected_target - expected_target * variance)
    self.assertLess(max(stable_set),
                    expected_target + expected_target * variance)
  ...
  def test_ignore_first_n_batch_size(self):
    ...
    for i in range(len(expected_data_timing)):
      self.assertAlmostEqual(expected_data_timing[i],
                             batch_estimator._data[i][1])
  ...
  def test_numpy_regression(self):
    ...
    self._run_regression_test(util._BatchSizeEstimator.linear_regression_numpy,
                              True)
...
class IdentityWindowTest(unittest.TestCase):
  ...
  def test_window_preserved(self):
    ...
    with TestPipeline() as pipeline:
      ...
      before_identity = (pipeline | 'start' >> beam.Create(data) |
    ...
    with self.assertRaisesRegex(ValueError, r'window.*None.*add_timestamps2'):
      with TestPipeline() as pipeline:
        ...
        assert_that(before_identity,
                    ...
                    reify_windows=True)
        after_identity = (
            before_identity | 'window' >> beam.WindowInto(
...
class ReshuffleTest(unittest.TestCase):
  ...
  def test_reshuffle_timestamps_unchanged(self):
    with TestPipeline() as pipeline:
      ...
      before_reshuffle = (
          pipeline | 'start' >> beam.Create(data) | 'add_timestamp' >>
  ...
  def test_reshuffle_window_fn_preserved(self):
    ...
    with TestPipeline() as pipeline:
      ...
      expected_windows = [
          ...
          [((1, 1), 1.0,
            IntervalWindow(1.0, 3.0)), ((2, 1), 1.0, IntervalWindow(1.0, 3.0)),
           ((3, 1), 1.0,
            IntervalWindow(1.0, 3.0)), ((1, 2), 2.0, IntervalWindow(2.0, 4.0)),
           ((2, 2), 2.0,
            IntervalWindow(2.0, 4.0)), ((1, 4), 4.0, IntervalWindow(4.0, 6.0))]
      ]
      ...
      before_reshuffle = (
          pipeline | 'start' >> beam.Create(data) |
  ...
  def test_reshuffle_global_window(self):
    ...
    before_reshuffle = (pipeline | beam.Create(data) |
                        beam.WindowInto(GlobalWindows()) | beam.GroupByKey() |
                        beam.MapTuple(lambda k, vs: (k, sorted(vs))))
    assert_that(before_reshuffle,
                equal_to(expected_data),
                label='before_reshuffle')
  ...
  def test_reshuffle_sliding_window(self):
    ...
    before_reshuffle = (pipeline | beam.Create(data) | beam.WindowInto(
        SlidingWindows(size=window_size, period=1)) | beam.GroupByKey() |
                        beam.MapTuple(lambda k, vs: (k, sorted(vs))))
    assert_that(before_reshuffle,
                equal_to(expected_data),
                label='before_reshuffle')
  ...
  def test_reshuffle_preserves_timestamps(self):
    with TestPipeline() as pipeline:
      ...
      before_reshuffle = (pipeline | beam.Create([
          {
              'name': 'foo',
      ...
      ]) | beam.Map(lambda element: beam.window.TimestampedValue(
      ...
      expected_data = [
          'MIN_TIMESTAMP - foo', 'Timestamp(0) - foo', 'Timestamp(33) - bar',
          'MAX_TIMESTAMP - bar'
      ]
      ...
      assert_that(formatted_before_reshuffle,
                  equal_to(expected_data),
                  label="formatted_before_reshuffle")
      assert_that(formatted_after_reshuffle,
                  equal_to(expected_data),
...
class GroupIntoBatchesTest(unittest.TestCase):
  ...
  def _create_test_data():
    scientists = [
        "Einstein", "Darwin", "Copernicus", "Pasteur", "Curie", "Faraday",
  ...
  def test_in_global_window(self):
    with TestPipeline() as pipeline:
      ...
      assert_that(
          ...
          equal_to([
              int(
                  math.ceil(GroupIntoBatchesTest.NUM_ELEMENTS /
...
class ReifyTest(unittest.TestCase):
  ...
  def test_timestamp_in_value(self):
    ...
    expected = [
        TestWindowedValue(('a', TimestampedValue(1, 100)), 100,
                          [GlobalWindow()]),
  ...
  def test_window_in_value(self):
    ...
    expected = [
        TestWindowedValue(('a', (1, 100, GlobalWindow())), 100,
                          [GlobalWindow()]),
...
class RegexTest(unittest.TestCase):
  ...
  def test_find_kv(self):
    with TestPipeline() as p:
      ...
      assert_that(pcol |
                  'key 1 group 1' >> util.Regex.find_kv('a (b) (c)', 1, 2),
                  equal_to([('b', 'c')]),
  ...
  def test_match_group_kv_pattern(self):
    with TestPipeline() as p:
      ...
      assert_that(pcol | 'key 1' >> util.Regex.matches_kv(rc, 1),
                  equal_to([("b", "a b c")]),
                  label="CheckKey1")

      assert_that(pcol | 'key 1 group 2' >> util.Regex.matches_kv(rc, 1, 2),
                  equal_to([("b", "c")]),
