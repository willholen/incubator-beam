...
class WindowTest(unittest.TestCase):
  ...
  def test_fixed_windows(self):
    ...
    self.assertEqual([IntervalWindow(12, 17)], windowfn.assign(context('v',
                                                                       12)))
    ...
    self.assertEqual([IntervalWindow(10, 15)], windowfn.assign(context('v',
                                                                       10)))
  ...
  def test_sessions_merging(self):
    ...
    self.assertEqual(
        [IntervalWindow(2, 12), IntervalWindow(19, 29)], merge(2, 19))

    self.assertEqual([IntervalWindow(2, 19)], merge(2, 9))
    self.assertEqual([IntervalWindow(2, 19)], merge(9, 2))

    self.assertEqual(
        [IntervalWindow(2, 19), IntervalWindow(19, 29)], merge(2, 9, 19))
    self.assertEqual(
        [IntervalWindow(2, 19), IntervalWindow(19, 29)], merge(19, 9, 2))
  ...
  def test_sliding_windows(self):
    with TestPipeline() as p:
      ...
      result = (pcoll | 'w' >> WindowInto(SlidingWindows(period=2, size=4)) |
                GroupByKey() | beam.MapTuple(lambda k, vs: (k, sorted(vs))) |
                reify_windows)
  ...
  def test_sessions(self):
    with TestPipeline() as p:
      ...
      result = (pcoll | 'w' >> WindowInto(Sessions(10)) | GroupByKey() |
                sort_values | reify_windows)
  ...
  def test_timestamped_value(self):
    with TestPipeline() as p:
      result = (p | 'start' >> Create([(k, k) for k in range(10)]) |
                Map(lambda x_t: TimestampedValue(x_t[0], x_t[1])) |
                'w' >> WindowInto(FixedWindows(5)) | Map(lambda v: ('key', v)) |
                GroupByKey() | beam.MapTuple(lambda k, vs: (k, sorted(vs))))
      assert_that(
          result, equal_to([('key', [0, 1, 2, 3, 4]), ('key', [5, 6, 7, 8,
                                                               9])]))

  def test_rewindow(self):
    with TestPipeline() as p:
      result = (
          p | Create([(k, k) for k in range(10)]) |
  ...
  def test_rewindow_regroup(self):
    with TestPipeline() as p:
      grouped = (p | Create(range(5)) |
                 Map(lambda t: TimestampedValue(('key', t), t)) |
                 'window' >> WindowInto(FixedWindows(5, offset=3)) |
                 GroupByKey() | MapTuple(lambda k, vs: (k, sorted(vs))))
      # Both of these group-and-ungroup sequences should be idempotent.
      regrouped1 = (grouped | 'w1' >> WindowInto(FixedWindows(5, offset=3)) |
                    'g1' >> GroupByKey() |
                    FlatMapTuple(lambda k, vs: [(k, v) for v in vs]))
      regrouped2 = (grouped | FlatMapTuple(lambda k, vs: [(k, v) for v in vs]) |
  ...
  def test_custom_windows(self):
    with TestPipeline() as p:
      ...
      result = (pcoll | 'custom window' >> WindowInto(TestCustomWindows()) |
                GroupByKey() |
  ...
  def test_window_assignment_idempotency(self):
    with TestPipeline() as p:
      ...
      result = (pcoll | 'window' >> WindowInto(FixedWindows(2)) |
...
class RunnerApiTest(unittest.TestCase):
  ...
  def test_windowfn_encoding(self):
    for window_fn in (GlobalWindows(), FixedWindows(37), SlidingWindows(2, 389),
                      Sessions(5077)):
