...
class PTransformTest(unittest.TestCase):
  ...
  def test_str(self):
    ...
    inputs_tr.inputs = ('ci',)
    self.assertEqual("<Impulse(PTransform) label=[Impulse] inputs=('ci',)>",
                     str(inputs_tr))
    ...
    self.assertEqual('<Impulse(PTransform) label=[Impulse] side_inputs=(4,)>',
                     str(side_tr))

    inputs_tr.side_inputs = ('cs',)
    self.assertEqual(
        """<Impulse(PTransform) label=[Impulse] """
        """inputs=('ci',) side_inputs=('cs',)>""", str(inputs_tr))
  ...
  def test_do_with_side_input_as_arg(self):
    with TestPipeline() as pipeline:
      ...
      pcoll = pipeline | 'Start' >> beam.Create([1, 2, 3])
      result = pcoll | 'Do' >> beam.FlatMap(lambda x, addon: [x + addon],
                                            pvalue.AsSingleton(side))
      assert_that(result, equal_to([11, 12, 13]))
  ...
  def test_undeclared_outputs(self):
    with TestPipeline() as pipeline:
      nums = pipeline | 'Some Numbers' >> beam.Create([1, 2, 3, 4])
      results = nums | 'ClassifyNumbers' >> beam.FlatMap(lambda x: [
          x,
  ...
  def test_multiple_empty_outputs(self):
    with TestPipeline() as pipeline:
      ...
      results = nums | 'ClassifyNumbers' >> beam.FlatMap(lambda x: [
          x, pvalue.TaggedOutput('even' if x % 2 == 0 else 'odd', x)
      ]).with_outputs()
  ...
  def test_do_fn_with_windowing_in_finish_bundle(self):
    ...
    pipeline = TestPipeline()
    result = (
        pipeline | 'Start' >> beam.Create([1]) | beam.ParDo(MyDoFn()) |
        WindowInto(windowfn) | 'create tuple' >>
        beam.Map(lambda v, t=beam.DoFn.TimestampParam, w=beam.DoFn.WindowParam:
                 (v, t, w.start, w.end)))
  ...
  def test_flatten_multiple_pcollections_having_multiple_consumers(self):
    with TestPipeline() as pipeline:
      ...
      even_length, odd_length = (input | beam.Map(split_even_odd).with_outputs(
  ...
  def test_group_by_key_input_must_be_kv_pairs(self):
    ...
    self.assertStartswith(
        e.exception.args[0], 'Input type hint violation at D: expected '
  ...
  def test_chained_ptransforms(self):
    with TestPipeline() as pipeline:
      t = (beam.Map(lambda x: (x, 1)) | beam.GroupByKey() |
  ...
  def test_multi_input_ptransform(self):
    ...
    class DisjointUnion(PTransform):
      ...
      def expand(self, pcollections):
        return (pcollections | beam.Flatten() | beam.Map(lambda x: (x, None)) |
                beam.GroupByKey() | beam.Map(lambda kv: kv[0]))
  ...
  def test_named_tuple(self):
    ...
    class MinMaxTransform(PTransform):
      ...
      def expand(self, pcoll):
        return MinMax(min=pcoll | beam.CombineGlobally(min).without_defaults(),
...
class PTransformLabelsTest(unittest.TestCase):
  ...
  def check_label(self, ptransform, expected_label):
    ...
    actual_label = sorted(label for label in pipeline.applied_labels
...
class PTransformTestDisplayData(unittest.TestCase):
  ...
  def test_map_named_function(self):
    ...
    expected_item = DisplayDataItem('len',
                                    key='fn',
  ...
  def test_map_anonymous_function(self):
    ...
    dd = DisplayData.create_from(tr)
    nspace = 'apache_beam.transforms.core.CallableWrapperDoFn'
    expected_item = DisplayDataItem('<lambda>',
                                    key='fn',
                                    label='Transform Function',
                                    namespace=nspace)
    hc.assert_that(dd.items, hc.has_item(expected_item))
  ...
  def test_flatmap_named_function(self):
    ...
    dd = DisplayData.create_from(tr)
    nspace = 'apache_beam.transforms.core.CallableWrapperDoFn'
    expected_item = DisplayDataItem('list',
                                    key='fn',
                                    label='Transform Function',
                                    namespace=nspace)
    hc.assert_that(dd.items, hc.has_item(expected_item))
  ...
  def test_filter_named_function(self):
    ...
    dd = DisplayData.create_from(tr)
    nspace = 'apache_beam.transforms.core.CallableWrapperDoFn'
    expected_item = DisplayDataItem('sum',
                                    key='fn',
                                    label='Transform Function',
                                    namespace=nspace)
    hc.assert_that(dd.items, hc.has_item(expected_item))
...
class PTransformTypeCheckTestCase(TypeHintTestCase):
  ...
  def test_do_fn_pipeline_pipeline_type_check_satisfied(self):
    ...
    d = (self.p | 'T' >> beam.Create([1, 2, 3]).with_output_types(int) |
  ...
  def test_do_fn_pipeline_pipeline_type_check_violated(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'T' >> beam.Create([1, 2, 3]).with_output_types(int) |
  ...
  def test_do_fn_pipeline_runtime_type_check_violated(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'T' >> beam.Create(['1', '2', '3']).with_output_types(str) |
       'Add' >> beam.ParDo(AddWithNum(), 5))
      self.p.run()
  ...
  def test_pardo_does_not_type_check_using_type_hint_decorators(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'S' >> beam.Create(['b', 'a', 'r']).with_output_types(str) |
       'ToStr' >> beam.FlatMap(int_to_str))

    self.assertEqual(
        "Type hint violation for 'ToStr': "
        "requires {} but got {} for a".format(int, str), e.exception.args[0])
  ...
  def test_pardo_does_not_type_check_using_type_hint_methods(self):
    ...
    self.assertEqual(
        "Type hint violation for 'Upper': "
        "requires {} but got {} for x".format(str, int), e.exception.args[0])
  ...
  def test_map_does_not_type_check_using_type_hints_methods(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'S' >> beam.Create([1, 2, 3, 4]).with_output_types(int) |
  ...
  def test_map_does_not_type_check_using_type_hints_decorator(self):
    ...
    self.assertEqual(
        "Type hint violation for 'Upper': "
        "requires {} but got {} for s".format(str, int), e.exception.args[0])
  ...
  def test_filter_does_not_type_check_using_type_hints_method(self):
    ...
    self.assertEqual(
        ...
        "requires {} but got {} for x".format(int, str), e.exception.args[0])
  ...
  def test_filter_does_not_type_check_using_type_hints_decorator(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'Ints' >> beam.Create([1, 2, 3, 4]).with_output_types(int) |
       'Half' >> beam.Filter(more_than_half))

    self.assertEqual(
        "Type hint violation for 'Half': "
        "requires {} but got {} for a".format(float, int), e.exception.args[0])
  ...
  def test_filter_type_checks_using_type_hints_decorator(self):
    ...
    (self.p | 'Str' >> beam.Create(range(5)).with_output_types(int) |
     'Half' >> beam.Filter(half) | 'ToBool' >>
     beam.Map(lambda x: bool(x)).with_input_types(int).with_output_types(bool))
  ...
  def test_group_by_key_only_output_type_deduction(self):
    d = (self.p |
         ...
         ('Pair' >> beam.Map(lambda x: (x, ord(x))).with_output_types(
             typing.Tuple[str, str])) | _GroupByKeyOnly())
  ...
  def test_group_by_key_output_type_deduction(self):
    d = (self.p | 'Str' >> beam.Create(range(20)).with_output_types(int) |
         ('PairNegative' >> beam.Map(lambda x: (x % 5, -x)).with_output_types(
             typing.Tuple[int, int])) | beam.GroupByKey())
  ...
  def test_group_by_key_only_does_not_type_check(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create([1, 2, 3]).with_output_types(int) |
       'F' >> _GroupByKeyOnly())

    self.assertEqual(
        "Input type hint violation at F: "
        "expected Tuple[TypeVariable[K], TypeVariable[V]], "
        "got {}".format(int), e.exception.args[0])
  ...
  def test_group_by_does_not_type_check(self):
    ...
    self.assertEqual(
        ...
        "expected Tuple[TypeVariable[K], TypeVariable[V]], "
        "got Iterable[int]", e.exception.args[0])
  ...
  def test_pipeline_checking_pardo_insufficient_type_information(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'Nums' >> beam.Create(range(5)) |
  ...
  def test_pipeline_checking_gbk_insufficient_type_information(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'Nums' >> beam.Create(range(5)).with_output_types(int) |
       'ModDup' >> beam.Map(lambda x: (x % 2, x)) | _GroupByKeyOnly())

    self.assertEqual(
        'Pipeline type checking is enabled, however no output '
        'type-hint was found for the PTransform '
        'ParDo(ModDup)', e.exception.args[0])
  ...
  def test_disable_pipeline_type_check(self):
    ...
    (self.p | 'T' >> beam.Create([1, 2, 3]).with_output_types(int) | 'Lower' >>
     beam.Map(lambda x: x.lower()).with_input_types(str).with_output_types(str))
  ...
  def test_run_time_type_checking_enabled_type_violation(self):
    ...
    (self.p | 'T' >> beam.Create(['some_string']) |
  ...
  def test_run_time_type_checking_enabled_types_satisfied(self):
    ...
    # run-time.
    result = (self.p | 'T' >> beam.Create(['t', 'e', 's', 't', 'i', 'n', 'g'
                                          ]).with_output_types(str) |
              'GenKeys' >> beam.Map(group_with_upper_ord) |
              'O' >> beam.GroupByKey() | _SortLists)
  ...
  def test_pipeline_checking_satisfied_but_run_time_types_violate(self):
    ...
    (self.p | 'Nums' >> beam.Create(range(5)).with_output_types(int) |
     'IsEven' >> beam.Map(is_even_as_key) | 'Parity' >> beam.GroupByKey())
  ...
  def test_pipeline_checking_satisfied_run_time_checking_satisfied(self):
    ...
    result = (self.p | 'Nums' >> beam.Create(range(5)).with_output_types(int) |
              'IsEven' >> beam.Map(is_even_as_key) |
              'Parity' >> beam.GroupByKey() | _SortLists)
  ...
  def test_pipeline_runtime_checking_violation_simple_type_input(self):
    ...
    # passed instead.
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create([1, 1, 1]) | ('ToInt' >> beam.FlatMap(
          lambda x: [int(x)]).with_input_types(str).with_output_types(int)))
      self.p.run()
  ...
  def test_pipeline_runtime_checking_violation_composite_type_input(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create([(1, 3.0), (2, 4.9), (3, 9.5)]) |
       ('Add' >> beam.FlatMap(lambda x_y: [x_y[0] + x_y[1]]).with_input_types(
  ...
  def test_pipeline_runtime_checking_violation_simple_type_output(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create([1, 1, 1]) | ('ToInt' >> beam.FlatMap(
          lambda x: [float(x)]).with_input_types(int).with_output_types(int)))
      self.p.run()
      ...
      (self.p | beam.Create([(1, 3.0), (2, 4.9), (3, 9.5)]) |
       ('Swap' >>
        beam.FlatMap(lambda x_y1: [x_y1[0] + x_y1[1]]).with_input_types(
            typing.Tuple[int, float]).with_output_types(typing.Tuple[float,
                                                                     int])))
      self.p.run()
  ...
  def test_combine_func_type_hint_does_not_take_iterable_using_decorator(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'M' >> beam.Create([1, 2, 3]).with_output_types(int) |
  ...
  def test_combine_runtime_type_check_satisfied_using_decorators(self):
    ...
    d = (self.p | 'K' >> beam.Create([5, 5, 5, 5]).with_output_types(int) |
  ...
  def test_combine_runtime_type_check_violation_using_decorators(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'K' >> beam.Create([5, 5, 5, 5]).with_output_types(int) |
       'Mul' >> beam.CombineGlobally(iter_mul))
      self.p.run()
  ...
  def test_combine_pipeline_type_check_using_methods(self):
    d = (self.p | beam.Create(['t', 'e', 's', 't']).with_output_types(str) |
  ...
  def test_combine_runtime_type_check_using_methods(self):
    ...
    d = (self.p | beam.Create(range(5)).with_output_types(int) |
  ...
  def test_combine_pipeline_type_check_violation_using_methods(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create(range(3)).with_output_types(int) |
       ('SortJoin' >> beam.CombineGlobally(lambda s: ''.join(sorted(s))).
        with_input_types(str).with_output_types(str)))

    self.assertEqual(
        "Input type hint violation at SortJoin: "
        "expected {}, got {}".format(str, int), e.exception.args[0])
  ...
  def test_combine_runtime_type_check_violation_using_methods(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create([0]).with_output_types(int) |
       ('SortJoin' >> beam.CombineGlobally(lambda s: ''.join(sorted(s))).
        with_input_types(str).with_output_types(str)))
      self.p.run()
  ...
  def test_combine_insufficient_type_hint_information(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'E' >> beam.Create(range(3)).with_output_types(int) |
       'SortJoin' >> beam.CombineGlobally(lambda s: ''.join(sorted(s))) |
       'F' >> beam.Map(lambda x: x + 1))

    self.assertStartswith(
        e.exception.args[0], 'Pipeline type checking is enabled, '
  ...
  def test_mean_globally_pipeline_checking_satisfied(self):
    d = (self.p | 'C' >> beam.Create(range(5)).with_output_types(int) |
  ...
  def test_mean_globally_pipeline_checking_violated(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'C' >> beam.Create(['test']).with_output_types(str) |
       'Mean' >> combine.Mean.Globally())
  ...
  def test_mean_per_key_pipeline_checking_violated(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create(map(str, range(5))).with_output_types(str) |
       ('UpperPair' >> beam.Map(lambda x: (x.upper(), x)).with_output_types(
           typing.Tuple[str, str])) | 'EvenMean' >> combine.Mean.PerKey())
      self.p.run()
  ...
  def test_mean_per_key_runtime_checking_satisfied(self):
    ...
    d = (self.p | beam.Create(range(5)).with_output_types(int) |
         ('OddGroup' >> beam.Map(lambda x: (bool(x % 2), x)).with_output_types(
             typing.Tuple[bool, int])) | 'OddMean' >> combine.Mean.PerKey())
  ...
  def test_mean_per_key_runtime_checking_violated(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create(range(5)).with_output_types(int) |
       ('OddGroup' >>
        beam.Map(lambda x: (x, str(bool(x % 2)))).with_output_types(
            typing.Tuple[int, str])) | 'OddMean' >> combine.Mean.PerKey())
      self.p.run()
  ...
  def test_count_globally_pipeline_type_checking_satisfied(self):
    d = (self.p | 'P' >> beam.Create(range(5)).with_output_types(int) |
  ...
  def test_count_perkey_pipeline_type_checking_satisfied(self):
    d = (self.p | beam.Create(range(5)).with_output_types(int) |
         ('EvenGroup' >> beam.Map(lambda x: (not x % 2, x)).with_output_types(
             typing.Tuple[bool, int])) | 'CountInt' >> combine.Count.PerKey())
  ...
  def test_count_perkey_pipeline_type_checking_violated(self):
    ...
    self.assertEqual(
        ...
        "but got {} for element".format(int), e.exception.args[0])
  ...
  def test_count_perkey_runtime_type_checking_satisfied(self):
    ...
    d = (self.p | beam.Create(['t', 'e', 's', 't']).with_output_types(str) |
         'DupKey' >> beam.Map(lambda x: (x, x)).with_output_types(
             typing.Tuple[str, str]) | 'CountDups' >> combine.Count.PerKey())
  ...
  def test_count_perelement_pipeline_type_checking_satisfied(self):
    d = (self.p | beam.Create([1, 1, 2, 3]).with_output_types(int) |
  ...
  def test_count_perelement_pipeline_type_checking_violated(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | 'f' >> beam.Create([1, 1, 2, 3]) |
       'CountElems' >> combine.Count.PerElement())

    self.assertEqual(
        'Pipeline type checking is enabled, however no output '
        'type-hint was found for the PTransform '
        'Create(f)', e.exception.args[0])
  ...
  def test_top_of_pipeline_checking_satisfied(self):
    d = (self.p | beam.Create(range(5, 11)).with_output_types(int) |
  ...
  def test_top_of_runtime_checking_satisfied(self):
    ...
    d = (self.p | beam.Create(list('testing')).with_output_types(str) |
  ...
  def test_per_key_pipeline_checking_violated(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create(range(100)).with_output_types(int) |
  ...
  def test_per_key_pipeline_checking_satisfied(self):
    d = (self.p | beam.Create(range(100)).with_output_types(int) |
         ('GroupMod 3' >> beam.Map(lambda x: (x % 3, x)).with_output_types(
             typing.Tuple[int, int])) | 'TopMod' >> combine.Top.PerKey(1))
  ...
  def test_sample_globally_pipeline_satisfied(self):
    d = (self.p | beam.Create([2, 2, 3, 3]).with_output_types(int) |
  ...
  def test_to_list_pipeline_check_satisfied(self):
    d = (self.p | beam.Create(
        (1, 2, 3, 4)).with_output_types(int) | combine.ToList())
  ...
  def test_to_list_runtime_check_satisfied(self):
    ...
    d = (self.p | beam.Create(list('test')).with_output_types(str) |
         combine.ToList())
  ...
  def test_to_dict_pipeline_check_violated(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      (self.p | beam.Create([1, 2, 3, 4]).with_output_types(int) |
       combine.ToDict())
    ...
    self.assertEqual(
        ...
        "but got Tuple[None, int] for element", e.exception.args[0])
  ...
  def test_inferred_bad_kv_type(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      _ = (self.p | beam.Create(['a', 'b', 'c']) |
           'Ungroupable' >> beam.Map(lambda x: (x, 0, 1.0)) | beam.GroupByKey())

    self.assertEqual(
        'Input type hint violation at GroupByKey: '
        'expected Tuple[TypeVariable[K], TypeVariable[V]], '
        'got Tuple[str, int, float]', e.exception.args[0])
...
class TestPTransformFn(TypeHintTestCase):
  ...
  def test_type_hints_arg(self):
    ...
    def MyTransform(pcoll, type_hints, test_arg):
      ...
      return (pcoll | beam.ParDo(lambda x: [x]).with_output_types(
