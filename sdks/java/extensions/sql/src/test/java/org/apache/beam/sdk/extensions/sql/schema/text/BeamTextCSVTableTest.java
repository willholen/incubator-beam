...
public class BeamTextCSVTableTest {
  ...
  @Test public void testBuildIOReader() {
    PCollection<BeamRecord> rows = new BeamTextCSVTable(buildBeamRecordType(),
  ...
  @Test public void testBuildIOWriter() {
    new BeamTextCSVTable(buildBeamRecordType(),
        readerSourceFile.getAbsolutePath()).buildIOReader(pipeline)
        .apply(new BeamTextCSVTable(buildBeamRecordType(), writerTargetFile.getAbsolutePath())
            .buildIOWriter());
    pipeline.run();
  ...
  private static BeamRecordType buildBeamRecordType() {
    return CalciteUtils.toBeamRowType(buildRelDataType());
  }

  private static BeamRecord buildRow(Object[] data) {
    return new BeamRecord(buildBeamRecordType(), Arrays.asList(data));
  }
}
